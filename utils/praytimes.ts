...
export class PrayTimes {
	...
	*/
	 ...
	 * 
	 */
	getTimes(date: Date | number[], coords: number[] | { lat: number, lng: number, elv?: number }, timezone?: number | "auto", dst?, format?) {
		
		this.lat = coords[0];
		this.lng = coords[1];
		this.elv = coords[2] ? coords[2] : 0;
		let timeFormat = format || this.timeFormat;
		if (date.constructor === Date)
			date = [(date as Date).getFullYear(), (date as Date).getMonth() + 1, (date as Date).getDate()];
	...
	gmtOffset(date: number[]): number {
		...
		var hoursDiff = (localDate.getTime() - GMTDate.getTime()) / (1000 * 60 * 60);
		return hoursDiff;
	}
